defmodule AzraReceiver.Hook.PushEvent do
  ...
  def as_docker_hub(%__MODULE__{request: request, target: target} = event) do
    push_data = Map.new |> Map.put("tag", target["tag"])
    repository = Map.new |> Map.put("repo_name", full_name(event))

    %{"push_data" => push_data, "repository" => repository}
  end


  def pretty_print(%__MODULE__{timestamp: timestamp, action: action, target: target, request: request} = event) do
